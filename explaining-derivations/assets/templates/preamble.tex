...
% \stepblock{n}{before}{after}{move}{licensed}{breaks if}{checked}{gloss}{unpacked}
%
% #9 is the sub-steps the expander was asked for. `registers.md` tells it that "a
% step that takes three moves to justify gets three sub-steps", the contract has
% a field for them, and the validator accepted them -- and nothing rendered them,
% so that instruction produced output that was thrown away. Empty when the
% expander sent none, which is the common case for a step that really is one move.
\newcommand{\stepblock}[9]{%
  ...
  {\footnotesize\color{smallprint}
   \begin{description}[leftmargin=6.4em, style=sameline, itemsep=1pt, topsep=0pt]
     ...
     \item[In words] #8
   \end{description}}
  \ifx\relax#9\relax\else
    {\footnotesize\color{smallprint}\vspace{2pt}
     \begin{description}[leftmargin=6.4em, style=sameline, itemsep=1pt, topsep=0pt]
       \item[Unpacked] \hfill
     \end{description}
     \vspace{-6pt}
     \begin{enumerate}[leftmargin=7.4em, nosep, label=\arabic*.]#9\end{enumerate}}
  \fi
  \end{tcolorbox}
}
...
% A step that *was* made explicit, but only under something the paper never
% states. Distinct from \stepgap on purpose: rendering it in the same red, under
% the same heading, contradicted the fully expanded block directly above it.
\newcommand{\stepcaveat}[3]{%
  \begin{tcolorbox}[breakable, enhanced, colback=white, colframe=gapsubstantive,
                    boxrule=0.6pt, left=6pt, right=6pt, top=5pt, bottom=5pt,
                    title={\small\bfseries\color{gapsubstantive} #1\hfill\normalfont\scriptsize rests on something the paper does not state}]
  {\footnotesize\textbf{What is missing.} #2\par\vspace{3pt}
   \textbf{What would close it.} #3}
  \end{tcolorbox}
}
