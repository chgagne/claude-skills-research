...
description: Use when working on a Digital Research Alliance of Canada (DRAC, Alliance, formerly Compute Canada) HPC cluster such as Narval, Rorqual, TamIA, Fir, Nibi or Trillium — including ssh to *.alliancecan.ca failing with "Permission denied (keyboard-interactive)", submitting or monitoring Slurm jobs (sbatch, squeue, sacct, job arrays, GPU allocations), staging data into /project or /scratch, or jobs stuck PENDING.
---
...
### How much actually varies — three clusters, same day, same user

This is why the principle is "discover, don't assume":

| | Narval | Rorqual | TamIA |
|---|---|---|---|
| Account | `def-x_cpu` / `def-x_gpu` | `def-x_cpu` / `def-x_gpu` | **`aip-x`** — one account, **no suffix** |
| Max walltime | 7d (`b5`) | 7d (`b5`) | **24h (`b3`)** |
| GPU partitions | `bygpu` + `bynode` | `bygpu` + `bynode` | **`bynode` only** |
| GPU gres | `gpu:a100`, `a100_3g.20gb` | `gpu:h100`, `nvidia_h100_80gb_hbm3_3g.40gb` | `gpu:h100`, `gpu:h200` |
| `/home` | 50 GB / 500K | 50 GB / 500K | **25 GiB / 250K** |
| `/scratch` | 20 TB | 20 TB | **2 TB** |
| `$HOME` | `/home/<user>` | `/home/<user>` | **`/home/c/<user>`** (sharded) |
| `seff` | yes | yes | **absent** |
