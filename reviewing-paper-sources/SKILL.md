...
description: Use when asked to review, critique, or assess an academic paper from its LaTeX sources — including pre-submission critique of a student's or collaborator's draft, or a referee report for a venue. Also when the only artifact is a blind PDF with no sources. Triggers on "review this paper", "assess the paper", "read my student's draft", "referee report", a directory holding main.tex/refs.bib/*.cls, a submission PDF to review, or a request to check a paper's claims, figures, bibliography or math.
---
...
For the not-yet-written third: empty sections, note-form prose and TODO stubs are normal at this stage and the authors already know about them; reporting them as defects wastes the review. What they cannot see is which of the text they *have* written is wrong. So give this third an inventory table — location, current state, and a yes/no column for whether it blocks submission — rather than prose. The yes/no column is the deliverable: on one draft it distinguished an empty Conclusions section and a section heading with nothing under it, both of which had to be written, from an appendix the author was mid-way through rewriting, which could ship as it stood. Read the notes rather than skipping them, too: an author's own `\todo{is this correct?}` against a method description means the method is not settled, which is a finding about the work and not about the prose — promote that one to content.

Two boundary calls worth making the same way each time. A venue-mandated statement that is only a stub is **container**, not not-yet-written, however unwritten it looks: writing it is mechanical and it earns no credit. An absent Limitations section is **content**, because what goes in it is the paper's own account of what its evidence cannot support, and that is a judgement only the authors can make.
...
**4a. Check baseline provenance.** Before comparing protocols, establish whether the baseline
rows were re-run or quoted. Take the reported values, pull that paper's own source
(`curl -sL arxiv.org/e-print/<id> | tar -xz`, or CVF/ACL Anthology for camera-readies), and
diff. An exact match across every column to two decimals settles it. Then read the baseline's
methods section and compare protocol and search budget against the submission's. On one review
this took fifteen minutes and produced the finding that set the rating. While you are there:
does every baseline named in a results table have a bibliography entry? On one paper four of
nine did not.
...
**Refute your own findings before committing the rating, not after.** Across four PDF-only
reviews the pass changed the **rating on three of them**, because the findings it overturned
were the ones carrying the recommendation. Dispatch one subagent with the
...
- **On an early draft, separate the two gates and let only one of them score.** The rule above was written for a finished paper, where a desk-reject risk and a soundness failure both stop the same submission. On a draft still being written they are different questions, and merging them produces a verdict about the *container* instead of the work. Anonymisation leaks, an unset template switch, page count, missing bibliography entries and LaTeX errors are all real and none can be skipped — and all of them are a morning's work that the authors were always going to do. State them, in their own section, under their own heading, listed after the content findings; give the §2 line as *two* lines — what gates the science, and what gates the submission — and score only the first. A review that opens with four mechanical items and one substantive one has told the author their paper is bad when what it means is that their build is unfinished. This was learned the hard way: a review on file did exactly that and had to be restructured after the author pointed it out.
...
- **Writing "the paper does not report X" after one failed search.** The largest source of
  wrong findings in PDF-only reviewing, six times across four reviews. Read the caption and
  heading inventory end to end, search the concept in several wordings, and read the whole
  subsection rather than the paragraph you landed on. Never write "I would accept this if the
  paper did Y" without checking that it has not already done Y.
- **Shipping a typographic finding straight from the text layer.** Extraction artifacts
  masquerade as author errors: a caption clipped out of the printed page, a hyphen removed at
  a line break, axis labels producing doubled words. Render the page and look before reporting.
...
`assets/`: `changes-preamble.tex`, `run-pdfshim.py` + `pdfshim/` (PDF -> pseudo-source)
`reference/`: `bibliography-audit.md`, `claim-audit.md`, `annotating-with-changes.md`,
`review-template.md`, `pdf-only-review.md` (no LaTeX sources)
